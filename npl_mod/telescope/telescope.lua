...
local function make_assertion(name, message, func)
    ...
    if type(message) == "function" then
        ...
        for _, _ in message:gmatch("%%s") do
            num_vars = num_vars + 1
        end
...
make_assertion(
    "gte",
    "'%s' to be greater than or equal to '%s'",
    function(a, b)
        return a >= b
    end
)
...
make_assertion(
    "lte",
    "'%s' to be less than or equal to '%s'",
    function(a, b)
        return a <= b
    end
)
make_assertion(
    "match",
    "'%s' to be a match for %s",
    function(a, b)
        return (tostring(b)):match(a)
    end
)
make_assertion(
    "nil",
...
make_assertion(
    "not_error",
    "result not to be an error",
    function(f)
        return not (not pcall(f))
    end
)
make_assertion(
    "not_match",
    "'%s' not to be a match for %s",
    function(a, b)
        return not (tostring(b)):match(a)
    end
)
make_assertion(
    "not_nil",
...
local function load_contexts(target, contexts)
    ...
    for _, v in ipairs(after_aliases) do
        env[v] = after_block
    end
    for _, v in ipairs(before_aliases) do
        env[v] = before_block
    end
...
local function run(contexts, callbacks, test_filter)
    ...
    local test_filter = test_filter or function(a)
            return a
        end
    ...
    local function invoke_callback(name, test)
        ...
        if type(callbacks[name]) == "table" then
            for _, c in ipairs(callbacks[name]) do
                c(test)
            end
    ...
    for i, v in filter(
        contexts,
        function(i, v)
            return v.test and test_filter(v)
        end
    ) do
...
local function test_report(contexts, results)
    ...
    for i, item in ipairs(contexts) do
        ...
        if previous_level ~= level and level == 0 then
            add_divider()
        end
        ...
        elseif results[i] then
            table.insert(
                buffer,
                function_name_format:format(space() .. name) .. status_format:format(results[i].status_label)
            )
        end
    end
    add_divider()
...
local function summary_report(contexts, results)
    ...
    for _, v in pairs(results) do
        ...
        if v.status_code == status_codes.err then
            r.errors = r.errors + 1
        elseif v.status_code == status_codes.fail then
            r.failed = r.failed + 1
        elseif v.status_code == status_codes.pass then
            r.passed = r.passed + 1
        elseif v.status_code == status_codes.pending then
            r.pending = r.pending + 1
        elseif v.status_code == status_codes.unassertive then
            r.unassertive = r.unassertive + 1
        end
    end
    local buffer = {}
